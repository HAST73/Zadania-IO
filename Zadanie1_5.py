...
# Obliczenie MSE (bledu sredniokwadratowego)
def mse(image1, image2):
    return np.mean((image1.astype(np.float32) - image2.astype(np.float32)) ** 2)

MSE_RGB = mse(image, image_reconstructed)
MSE_Y = mse(Y, YCC_transmitted[:, :, 0])
MSE_Cb = mse(Cb, Cb_up)
MSE_Cr = mse(Cr, Cr_up)

# Wyswietlenie wyników MSE
print(f"Błąd MSE dla całego obrazu RGB: {MSE_RGB:.2f}")
print(f"Błąd MSE dla kanału Y: {MSE_Y:.2f}")
print(f"Błąd MSE dla kanału Cb: {MSE_Cb:.2f}")
print(f"Błąd MSE dla kanału Cr: {MSE_Cr:.2f}")
